...
export function initializeAdmin(){if(!db.prepare('SELECT id FROM admin WHERE id=1').get()){const p=process.env.ADMIN_PASSWORD||randomBytes(18).toString('base64url');setPassword(p);if(!process.env.ADMIN_PASSWORD)writeFileSync(path.join(dataDir,'admin-access.txt'),`Hind Lux Shop — accès local\nAdresse : /admin\nMot de passe : ${p}\nChangez ce mot de passe dans Paramètres > Sécurité.\n`,{mode:0o600});console.log('Compte administrateur créé. Accès local : data/admin-access.txt (ou ADMIN_PASSWORD).');}}
