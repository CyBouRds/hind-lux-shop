...
export default function Intro({onFinish}){
  ...
  return <section className={'intro '+(exiting?'intro-exit':'')} role="dialog" aria-modal="true" aria-label={t('Bienvenue chez Hind Lux Shop')}><video ref={video} src="/media/hind-intro-1080p.mp4?v=20260914-clean" autoPlay muted={muted} playsInline preload="auto" onEnded={finish} onError={finish} onTimeUpdate={e=>setProgress(e.currentTarget.currentTime/(e.currentTarget.duration||4.06))}/><div className="intro-top"><span>HIND LUX SHOP</span><LanguageSwitch/></div><div className="intro-bottom"><button className="intro-audio" onClick={()=>setMuted(!muted)} aria-label={t(muted?'Activer le son':'Couper le son')}>{muted?<VolumeX size={19}/>:<Volume2 size={19}/>}</button><button className="intro-skip" onClick={finish}>{t('Passer l’introduction')} <ArrowRight size={18}/></button></div><div className="intro-progress" style={{transform:`scaleX(${progress})`}}/><div className="intro-seam" aria-hidden="true"/></section>;
}
